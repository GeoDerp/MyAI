...
- Provenance bundles and partial artifacts: written to `MYAI_PARTIAL_DIR` (default `/tmp`) when no external provenance store is configured.
...
Quick env var cheat sheet for Copilot helpers

- `MYAI_PARTIAL_DIR` — directory for provenance bundles and partial LLM outputs (default `/tmp`).
- `MYAI_RENDERED_OUTPUT` — path to save the final rendered HTML snapshot from the web UI (default `/tmp/research_report.html`).
- `LANGGRAPH_URL` — when set, the agent will attempt to persist sources to a LangGraph instance; otherwise provenance bundles are written to `MYAI_PARTIAL_DIR`.
- `TORM_URL` — optional TORM service for expanding questions into perspectives.
- `EXA_API_KEY` — optional API key used by the Exa adapter; when present `academic_retrieval.search_with_fallback` may call `exa_search` as a retrieval fallback.

When writing tests or helping users, prefer patching or setting these env vars in the test environment so that Copilot-driven edits don't accidentally attempt live network calls.

Test-friendly example
---------------------
When running unit tests or executing Copilot-driven changes locally, set environment variables to safe temporary locations. Example (bash):

```bash
export MYAI_PARTIAL_DIR=$(mktemp -d)
export MYAI_RENDERED_OUTPUT="$MYAI_PARTIAL_DIR/research_report.html"
# Run tests
pytest tests/test_webui.py::test_webui_post_writes_html_and_provenance -q
```

This ensures provenance bundles and rendered HTML are written into an isolated temporary directory that your test harness can inspect or delete.
